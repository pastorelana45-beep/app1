...
const App: React.FC = () => {
  ...
  // NUOVA FUNZIONE PER IL TASTO PLAY
  const handlePlayLast = () => {
    if (lastSequence.length === 0) {
      alert("Registra prima qualcosa!");
      return;
    }
    // @ts-ignore - aggiungeremo playSequence nell'AudioEngine nel prossimo step
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-8 py-8 space-y-8 pb-32">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-6">
            ...
            {/* GRIGLIA PULSANTI AGGIORNATA CON PLAY */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <button onClick={handleRec} className={`p-6 rounded-[2rem] border transition-all flex flex-col items-center justify-center gap-3 ${appState === 'recording' ? 'bg-red-500/10 border-red-500/40' : 'bg-white/5 border-white/5'}`}>
              ...
              <button onClick={handleLiveMode} className={`p-6 rounded-[2rem] border transition-all flex flex-col items-center justify-center gap-3 ${appState === 'live' ? 'bg-purple-500/10 border-purple-500/40' : 'bg-white/5 border-white/5'}`}>
              ...
              <button onClick={handleExport} className="p-6 rounded-[2rem] bg-white/5 border border-white/5 flex flex-col items-center justify-center gap-3 relative">
