...
import { 
  Mic, Square, Lock, Save, Activity, Sliders, ChevronUp, ChevronDown, Play
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [sensitivity, setSensitivity] = useState(0.002); // Default più sensibile
          ...
          <div className="flex items-center gap-4">
            <div className="flex flex-col items-end mr-4">
              ...
              <div className="flex items-center gap-2 bg-white/5 rounded-xl p-1 border border-white/5">
                <button onClick={() => handleOctaveChange(-1)} className="p-1 hover:text-purple-500"><ChevronDown className="w-4 h-4" /></button>
                <span className="text-xs font-black w-4 text-center">{octaveShift}</span>
                <button onClick={() => handleOctaveChange(1)} className="p-1 hover:text-purple-500"><ChevronUp className="w-4 h-4" /></button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <button onClick={handleRec} className={`p-6 rounded-[2rem] border transition-all flex flex-col items-center justify-center gap-3 ${appState === 'recording' ? 'bg-red-500/10 border-red-500/40' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}>
              ...
              <button 
                ...
                className={`p-6 rounded-[2rem] border transition-all flex flex-col items-center justify-center gap-3 bg-white/5 border-white/5 ${lastSequence.length === 0 ? 'opacity-20' : 'hover:bg-white/10 active:scale-95'}`}
              >
              ...
              <button onClick={handleLiveMode} className={`p-6 rounded-[2rem] border transition-all flex flex-col items-center justify-center gap-3 ${appState === 'live' ? 'bg-purple-500/10 border-purple-500/40' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}>
              ...
              <button onClick={handleExport} className="p-6 rounded-[2rem] bg-white/5 border border-white/5 flex flex-col items-center justify-center gap-3 relative hover:bg-white/10">
          ...
          <aside className="lg:col-span-4 glass p-8 rounded-[3rem] flex flex-col items-center justify-center text-center border-white/5 relative h-full min-h-[400px]">
             ...
             <div className="mt-12 w-full space-y-4">
               <span className="text-[9px] font-black opacity-30 uppercase tracking-widest">Sensibilità</span>
               <div className="pt-2">
                 <input 
                   type="range" 
                   min="0.0001" 
