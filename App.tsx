...
const App: React.FC = () => {
  ...
  const [bluetoothFix, setBluetoothFix] = useState(false);
  ...
  const toggleBluetoothFix = () => {
    const newVal = !bluetoothFix;
    setBluetoothFix(newVal);
    audioEngineRef.current?.setBluetoothMode(newVal);
  };
    ...
    <div className="min-h-screen bg-[#050507] text-white flex flex-col">
            ...
            <div className="space-y-6 pt-6 border-t border-white/5">
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="text-[10px] font-black uppercase tracking-widest text-white/40">Bluetooth Fix</span>
                  <span className="text-[9px] text-purple-400/60 font-mono">Anti-Call Mode (HI-FI)</span>
                </div>
                <button 
                  onClick={toggleBluetoothFix}
                  className={`p-3 rounded-xl border transition-all ${bluetoothFix ? 'bg-purple-600 border-purple-500 text-white shadow-lg' : 'bg-white/5 border-white/5 text-white/20'}`}
                >
                  <Bluetooth className={`w-4 h-4 ${bluetoothFix ? 'animate-pulse' : ''}`} />
                </button>
              </div>
      ...
      <footer className="p-6 border-t border-white/5 flex justify-between items-center bg-black/40 backdrop-blur-xl">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            <Zap className={`w-3 h-3 ${bluetoothFix ? 'text-purple-400' : 'text-white/20'}`} />
            <span className="text-[9px] font-black uppercase tracking-widest text-white/40">
              Audio Profile: {bluetoothFix ? 'Studio (Hi-Fi)' : 'Low Latency'}
            </span>
          </div>
        </div>
